...
app.get('/api/attendance/stats', async (req, res) => {
  try {
    ...
    const allToday = await Attendance.find({ date: today }).select("userId");
    const uniqueUserIds = new Set(allToday.map(a => String(a.userId)));
    const presentToday = uniqueUserIds.size;
