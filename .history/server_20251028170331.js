...
app.get('/api/attendance/stats', async (req, res) => {
  try {
   ...
   const presentTodayResult = await Attendance.aggregate([
  { $match: { date: today } },
  {
    $group: {
      _id: { $toString: "$userId" } // normalize to string before grouping
    }
  },
  { $count: "count" }
]);

const presentToday = presentTodayResult.length > 0 ? presentTodayResult[0].count : 0;
    ...
    res.json({
      ...
      holidayToday: holidayToday
        ? { date: holidayToday.date, name: holidayToday.name, reason: holidayToday.reason }
        : null
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
